...
struct State<'a> {
    ...
    shadow_debug_depth_pass: depth_pass::DepthPass,
    obj_model: Model,
...
impl<'a> State<'a> {
    async fn new(window: &'a Window) -> State<'a> {
        ...
        let shadow_debug_depth_pass = depth_pass::DepthPass::new(
            &device,
            texture::SHADOW_MAP_SIZE,
            texture::SHADOW_MAP_SIZE,
            config.format,
    ...
    pub fn resize(&mut self, new_size: winit::dpi::PhysicalSize<u32>) {
        if new_size.width > 0 && new_size.height > 0 {
            ...
            self.shadow_debug_depth_pass
                .resize(&self.device, SHADOW_MAP_SIZE, SHADOW_MAP_SIZE);
